...
@command_hook(['pick', 'choose', 'choice'], help=".choice <something> <something else> [third choice] ... -- Makes a choice for you")
...
@command_hook(['rand', 'random'], help=".random [arg1] [arg2] ... -- Picks a random number between arg1 and arg2 (If one argument is missing it assumes the only argument is the upper limit and the lower is 1, and if both arguments are missing it generates a number between 1 and 10)")
def rand(irc, event):
    bottom = 1
    top = 10
    if len(event.args):
